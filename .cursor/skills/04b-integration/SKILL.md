...
flowchart TD
    ...
    prereq -->|No| block[Trigger 04a and 04 first]
    prereq -->|Yes| accept[Accept Handoff Verify No-Go + Read-Back]
    accept --> hi_check[Check human-interventions/active/ for phase:04b-integration items]
    ...
    s5 --> gate{Gate 4b Human Approval}
...
1. Read the handoff packages from Phase 04a (Backend Implementation) and Phase 04 (Frontend Development)
...
3. Verify all No-Go items pass (interpret "P0" as MVP scope when in MVP mode):
   ...
   - [ ] Frontend has API client structure (see [dev-standards.md](../04-frontend-development/dev-standards.md) → API Integration)
...
6. Only after all above: begin Phase 04b work.
...
1. Check `human-interventions/active/` for files tagged `phase: 04b-integration` or `phase: all`
...
- **From 04a-backend-implementation:** If API changes, re-run contract verification and client integration
- **From 04-frontend-development:** If frontend API usage changes, re-verify contract
...
- [dev-standards.md](../04-frontend-development/dev-standards.md) — frontend API client structure
